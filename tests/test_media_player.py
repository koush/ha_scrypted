...
async def test_intercom_media_player_created(
    ...
    state = hass.states.get("media_player.doorbell_intercom")
    assert state is not None
    assert state.state == "idle"
    # cam1 has no Intercom interface -> no media player
    assert hass.states.get("media_player.front_door_cam_intercom") is None
...
async def test_play_media_url_starts_intercom(
    ...
    await hass.services.async_call(
        ...
        "play_media",
        {
            "entity_id": "media_player.doorbell_intercom",
    ...
    assert hass.states.get("media_player.doorbell_intercom").state == "playing"
    ...
    assert hass.states.get("media_player.doorbell_intercom").state == "idle"
...
async def test_play_media_unavailable_device_raises(
    ...
    entity = component.get_entity("media_player.doorbell_intercom")
